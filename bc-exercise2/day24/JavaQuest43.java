...
public class JavaQuest43 {
  ...
  public static int findCenter(int[][] edges) {
    int n = edges.length + 1; // Number of nodes in the graph , connect to center node
    System.out.println("edge= " + edges.length);

    // Create a frequency map to count the occurrences of each node
    ...
    for (int[] edge : edges) {
        ...
        // Check if any node has occurred twice
        if (frequency[u] == 2) {
            return u;
        }
        if (frequency[v] == 2) {
    ...
    return -1; // Return -1 if the given edges do not form a valid star graph
  }
